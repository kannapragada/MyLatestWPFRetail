USE [NewAppDb]
GO
/****** Object:  StoredProcedure [dbo].[GetUserSaleDetails]    Script Date: 04/16/2015 18:34:35 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[GetUserSaleDetails]            
...
BEGIN            
 ...
 IF (@Sales_Id = 'ALL')            
	BEGIN            
		SELECT distinct s.Sales_Id, s.Sales_User_Id, s.Sales_Total_Sales_Amt, s.Sales_Total_Disc_Amt,         
		s.Sales_Total_Tax_Amt, s.Sales_Final_Sales_Amt, s.Sales_Amt_Paid, s.Sales_Balance_Amt,         
		s.Sales_Create_Date, s.Sales_Last_Mod_Date, c.User_Name        
		FROM Sales AS s         
		INNER JOIN SalesDetails AS sd ON s.Sales_Id = sd.SD_Sales_Id         
		INNER JOIN UserProfile AS c ON s.Sales_User_Id = c.User_Id          
 ...
 BEGIN            
	...
	SET @SQL = 'SELECT distinct s.Sales_Id, s.Sales_User_Id, s.Sales_Total_Sales_Amt, s.Sales_Total_Disc_Amt,           
    s.Sales_Total_Tax_Amt, s.Sales_Final_Sales_Amt, s.Sales_Amt_Paid, s.Sales_Balance_Amt,         
    s.Sales_Create_Date, s.Sales_Last_Mod_Date, c.User_Name        
	FROM Sales AS s         
	INNER JOIN SalesDetails AS sd ON s.Sales_Id = sd.SD_Sales_Id         
	...
	BEGIN              
		if (LEN(@tmpstr) > 0)              
			...
			SET @tmpstr = ' WHERE '              
		set @tmpstr = @tmpstr + '(c.User_Id = ''' + CAST(@User_Id AS NVARCHAR(15))  + ''')'                
	END          
	...
	BEGIN              
		...
		set @tmpstr = @tmpstr + '(c.User_Name = ''' + CAST(@User_Name AS NVARCHAR(15))  + ''')'                  
	END              
