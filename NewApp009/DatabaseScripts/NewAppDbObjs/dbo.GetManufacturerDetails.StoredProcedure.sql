USE [NewAppDb]
GO
/****** Object:  StoredProcedure [dbo].[GetManufacturerDetails]    Script Date: 04/16/2015 18:34:35 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[GetManufacturerDetails]
	@Manuf_Id		varchar(15)
AS
...
BEGIN
	...
	SET @sqlerror = ''
		...
		BEGIN TRY		
				select * from Manufacturer where Manuf_Id = @Manuf_Id
		...
		FROM	Sales AS s INNER JOIN
				...
				Manufacturer AS m ON m.Manuf_Id = ib.IB_Manuf_Id
		WHERE	m.Manuf_Id = @Manuf_Id
		...
		SELECT	sd.SD_Sales_Id, sd.SD_Serial_Numb, 
				...
				sd.SD_Create_Date, sd.SD_Last_Mod_Date, i.Item_Name, m.Manuf_Id, m.Manuf_Name
		FROM	Sales AS s INNER JOIN
				SalesDetails AS sd ON s.Sales_Id = sd.SD_Sales_Id INNER JOIN
				Items AS i ON sd.SD_Item_Id = i.Item_Id INNER JOIN
				ItemBatch as ib ON sd.SD_Item_Id = ib.IB_Item_Id AND sd.SD_Batch_Id = ib.IB_Batch_Id  INNER JOIN
				Manufacturer AS m ON m.Manuf_Id = ib.IB_Manuf_Id
		WHERE	s.Sales_Cust_Id = @Manuf_Id
		END TRY

		-- Whoops, there was an error
		BEGIN CATCH
					SET @sqlerror = 'Error while getting the Manufacturer details. ' + ERROR_MESSAGE()
