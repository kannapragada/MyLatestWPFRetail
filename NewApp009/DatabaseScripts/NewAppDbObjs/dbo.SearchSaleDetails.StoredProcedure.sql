USE [NewAppDb]
GO
/****** Object:  StoredProcedure [dbo].[SearchSaleDetails]    Script Date: 04/16/2015 18:34:35 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[SearchSaleDetails]          
...
BEGIN          
 ...
 BEGIN          
  ...
  SET @tmpstr = ' WHERE (s.Sales_Id like ''%' + CAST(@Sales_Id AS NVARCHAR(15)) + '%''' + ')'          
 ...
 IF (@Cust_Id IS NOT NULL AND LEN(@Cust_Id) > 0)            
  ...
  BEGIN            
   ...
   if (LEN(@tmpstr) > 0)            
    ...
    set @tmpstr = @tmpstr + '(c.Cust_Id like ''%' + CAST(@Cust_Id AS NVARCHAR(15)) + '%''' + ')'            
  ...
  BEGIN            
   ...
   if (LEN(@tmpstr) > 0)            
    ...
    set @tmpstr = @tmpstr + '(c.Cust_Name like ''%' + CAST(@Cust_Name AS NVARCHAR(15)) + '%''' + ')'            
...
IF ((IsDate(@Sale_Start_Date) = 1) AND (IsDate(@Sale_End_Date) = 1))          
  ...
  BEGIN          
   ...
   set @tmpstr = @tmpstr + '(Sales_Create_Date BETWEEN ''' + CONVERT(varchar(30), @Sale_Start_Date, 126) + ''' AND ''' + CONVERT(varchar(30), @Sale_End_Date, 126) + ''')'          
