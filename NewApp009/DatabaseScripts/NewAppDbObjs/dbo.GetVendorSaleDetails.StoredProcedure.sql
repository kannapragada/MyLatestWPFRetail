USE [NewAppDb]
GO
/****** Object:  StoredProcedure [dbo].[GetVendorSaleDetails]    Script Date: 04/16/2015 18:34:35 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[GetVendorSaleDetails]                
...
BEGIN                
 ...
 BEGIN            
  ...
  SET @SQL = 'SELECT distinct sd.SD_Sales_Id SalesId, sd.SD_Serial_Numb,               
     ...
     INNER JOIN ItemBatch AS ib ON IB.IB_Item_Id = SD.SD_Item_Id AND ib.IB_Batch_Id = sd.SD_Batch_Id    
     INNER JOIN Items AS i ON i.Item_Id = ib.IB_Item_Id    
     INNER JOIN Vendor AS m ON m.Vendor_Id = ib.IB_Vendor_Id'    
  ...
  BEGIN                  
   ...
   SET @tmpstr = ' WHERE '                  
   set @tmpstr = @tmpstr + '(m.Vendor_Id = ''' + CAST(@Vendor_Id AS NVARCHAR(15))  + ''')'                    
  END              
  ...
  BEGIN                  
   ...
   set @tmpstr = @tmpstr + '(m.Vendor_Name = ''' + CAST(@Vendor_Name AS NVARCHAR(15))  + ''')'                      
  END                  
